...
public class ReplicatedCachePresenter
        extends CachePresenter<ReplicatedCachePresenter.MyView, ReplicatedCachePresenter.MyProxy>
        implements SupportsExpertMode {

    @Inject
    public ReplicatedCachePresenter(EventBus eventBus,
            MyView view,
            MyProxy myProxy,
            Finder finder,
    ...
    // @formatter:off
    @ProxyCodeSplit
    @Requires(REPLICATED_CACHE_ADDRESS)
    @NameToken(NameTokens.REPLICATED_CACHE)
    public interface MyProxy extends ProxyPlace<ReplicatedCachePresenter> {
    }

    public interface MyView extends CacheView<ReplicatedCachePresenter> {
    }
    // @formatter:on
}
